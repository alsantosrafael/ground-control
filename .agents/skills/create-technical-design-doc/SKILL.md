...
| **Architecture**  | Components, data flow         | "Frontend → Spring API → Service → Stripe → Database"           |
...
| Category                 | Avoid                                    | Why                                               |
| ------------------------ | ---------------------------------------- | ------------------------------------------------- |
| **CLI Commands**         | `./gradlew flywayMigrate`, `kubectl rollout undo` | Too specific, may change with tooling      |
| **Code Snippets**        | Java/Spring implementation                      | Belongs in code, not docs                  |
| **Framework Specifics**  | `@Service`, `extends JpaRepository`            | Framework may change, decision is what matters |
| **File Paths**           | `src/main/java/.../BackfillJob.java`           | Implementation detail, not architectural decision |
| **Tool-Specific Syntax** | Spring annotations, JPA entity mappings         | Document pattern, not implementation       |
...
**Goal**: TDD should survive implementation changes. If you migrate from Spring Boot to Micronaut, or from JPA to jOOQ, the TDD should still be valid.

## Document Structure

### Mandatory Sections (Must Have)

These sections are **required**. If the user doesn't provide information, you **must ask** via clarifying questions in natural conversation:
...
Collect basic information using structured questions in natural conversation:
...
**Ask in natural conversation IN THE USER'S LANGUAGE**:
...
| **Phase 2 - Core**    | Entities & repos  | Create JPA entities, Spring Data repositories | @Dev2   | TODO   | 3d  |
|                       | Services          | Implement business logic services      | @Dev2   | TODO   | 4d       |
| **Phase 3 - APIs**    | REST endpoints    | Create Spring MVC controllers and request/response DTOs | @Dev3 | TODO | 3d |
...
| --------------------- | ------------------------ | ------------------------ | -------------------- |
| **Unit Tests**        | Services, repositories   | > 80%                    | JUnit 5 + Mockito                     |
| **Integration Tests** | API endpoints + database | Critical paths           | Spring Boot Test + MockMvc/WebTestClient + Testcontainers |
| **E2E Tests**         | Full user flows          | Happy path + error cases | RestAssured or Cypress (if UI involved) |
...
1. Export data from old system (batch job or SQL export)
2. Transform data to new schema (Spring Batch or ETL pipeline)
3. Validate data integrity (checksums, row counts)
4. Load into new system (batch job, migration tool, or SQL import)
...
If a Confluence publishing skill/tool is available in the runtime, use it to publish. Otherwise, provide the final Markdown ready to paste.

## Ground Control-Specific Considerations

When creating TDDs for Ground Control projects, consider:

### Architecture Alignment

- **Module Boundaries**: Does this create a new module or extend existing?
- **State Isolation**: Each module should have its own database
- **Communication**: Inter-module communication via HTTP, events, or published interfaces (never direct DB)

### Entity Naming

**CRITICAL**: Always use module-prefixed entity names in database schema section:
...
- `BillingCustomer` (NOT `Customer`)
- `BillingSubscription` (NOT `Subscription`)
- `BillingPayment` (NOT `Payment`)
```
...
- [Spring Modulith Docs](../../../build/spring-modulith-docs/all-docs.adoc)
...
Add section for architecture compliance:

```markdown
## Architecture Compliance

- [ ] Module has dedicated database (State Isolation - Principle 8)
- [ ] No cross-module entity imports (Independence - Principle 3)
- [ ] Uses HTTP/events/published interfaces for inter-module communication (Explicit Communication - Principle 5)
- [ ] Entities have module-prefixed names (e.g., `BillingCustomer`)
...
- Verify each module uses its own datasource
- Confirm `@Transactional` boundaries use the correct module datasource/transaction manager
- Review controller complexity (keep lean, delegate to services)
```
